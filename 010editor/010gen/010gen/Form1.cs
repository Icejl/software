...
namespace _010gen
{
    public partial class Form1 : Form
    {
        ...
        public uint sub_14036D600(string username, int flag, uint a3,uint a4)
        {
            ...
            {
                uint idx = (uint)(username[i] & 0xff);
                uint v1 = table[j];
                uint v2 = result + table[idx];
                uint v3 = table[k];
                if (flag != 0)
                {
        ...
        public void gen2(string username)
        {
            byte [] pwd = new byte[10];

            uint v22 = 0x8000; // 代码有效时间 * 86400
            uint a48 = 100; // 注册码可用次数？
            uint v25 = sub_14036D600(username, 1, v22, a48);

            pwd[3] = 0xac; // 0xac 注册方式 固定
            pwd[4] = (byte)(v25 & 0xff);
            pwd[5] = (byte)((v25 >> 8) & 0xff);
            pwd[6] = (byte)((v25 >> 16) & 0xff);
            pwd[7] = (byte)((v25 >> 24) & 0xff);

            // t = (pwd[6] ^ pwd[0]) + ( (pwd[5] ^ pwd[9]) << 16 ) + ( (pwd[8] ^ pwd[4]) << 8 )
            uint t = (((v22 * 0x11) ^ 0xFFE53167) + 180597) ^ 0x22C078 ^ 0x5B8C27;

            pwd[0] = (byte)((t & 0xff) ^ pwd[6]);
            pwd[9] = (byte)(((t >> 16) & 0xff) ^ pwd[5]);
            pwd[8] = (byte)(((t >> 8) & 0xff) ^ pwd[4]);

            // t = (pwd[5]^pwd[2]) + ((pwd[7]^pwd[1]) << 8)
            t = (((a48 * 0xb) ^ 0x3421) + 0x10000 - 19760) ^ 0x7892;

            pwd[2] = (byte)((t & 0xff) ^ pwd[5]);
            pwd[1] = (byte)(((t >> 8) & 0xff) ^ pwd[7]);

            StringBuilder sb = new StringBuilder();
            for ( int i = 0; i < 10; i++ )
            {
                sb.AppendFormat("{0:x2}", pwd[i]);
                if ( i == 1 || i == 3 || i == 5 || i == 7 )
                {
                    sb.Append("-");
                }
        ...
        {
            if ( txt_name.Text == "999")
            {
                txt_key.Text = "不可以为999";
            }
            else
            {
                gen2(txt_name.Text);
